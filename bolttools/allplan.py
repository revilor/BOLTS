...
class AllplanBase(BaseElement):
	def __init__(self,basefile,collname,backend_root):
		check_schema(basefile,"allplanbase",
...
class AllplanData(DataBase):
	def __init__(self,repo):
		DataBase.__init__(self,"allplan",repo)
		self.bases = []
		...
		for coll in listdir(self.backend_root):
			...
			for basefile in base_info:
				base = AllplanBase(basefile,coll,self.backend_root)
				self.bases.append(base)
				self.collection_bases.add_link(repo.collections[coll],base)

				for id in basefile["classids"]:
					...
					self.base_classes.add_link(base,repo.classes[id])
